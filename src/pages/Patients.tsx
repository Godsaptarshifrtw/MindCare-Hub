...
export default function Patients() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        // Primary: try 'patients' collection
        const snap = await getDocs(collection(db, 'patients'));
        let rows = snap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));
        // Fallback: use 'patientProfiles' if primary is empty
        if (rows.length === 0) {
  ...
  // Fetch per-patient summaries: appointment count and profile presence
  useEffect(() => {
    async function loadSummaries(items: Patient[]) {
      const limitCount = 24; // limit to avoid excessive reads
